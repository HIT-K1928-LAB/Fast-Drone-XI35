"""Gazebo SITL adapter. Idle until a goal; --execute enables PX4Ctrl output."""
import os
...
workspace = Path(os.environ.get("FD_WS", "/root/Fast-Drone-XI35/Fast-Drone-XI35"))
...
class CheckedPublisher:
    ...
    def publish(self, msg):
        ...
        if np.linalg.norm(pos - self.owner._odom_pos()) > 1.0:
            self.owner.stop("reference tracking error exceeds 1 m")
            return
...
class GazeboYopo(upstream.YopoNet):
    def __init__(self, execute):
        ...
        self.state_sub = rospy.Subscriber("/iris_0/mavros/state", State, self.receive_state, queue_size=1)
        ...
        rospy.logwarn("SITL adapter: output=%s. WAITING FOR GOAL; no automatic movement.", output)
        settings = dict(use_tensorrt=False, goal=[0, 0, 1.2], topk=1,
                        pitch_angle_deg=0, odom_topic="/gazebo/iris_0/odometry",
                        depth_topic="/iris_0/stereo_camera/depth/image_raw",
                        ctrl_topic=output, plan_from_reference=True, verbose=False)
        super().__init__(settings, str(yopo_dir / "saved/YOPO_1/epoch50.pth"))
    ...
    def input_problem(self):#检查问题
        ...
        if self.latest_depth_shape != (640, 370):
            return "expected calibrated depth size 640x370"
        ...
        for stamp in (self.odom.header.stamp, self.latest_depth_stamp):
            ...
            if age < -0.05 or age > 0.5:
                return "sensor timestamp out of range"
        if self.execute and (now - self.state_received > 3.0 or not self.fcu.connected
    ...
    def callback_set_goal(self, msg):#收到目标进行安全检查，通过则启动YOPO任务
        with self.guard:
            ...
            if problem:
                ...
                return
            if msg.header.frame_id not in ("world", "yopo_preview"):
                rospy.logwarn("Goal frame must be world or the identity frame yopo_preview.")
                return
            p = self._odom_pos()
            if p[2] < 0.6 or np.linalg.norm(self._odom_vel()) > 0.15:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--execute", action="store_true", help="Enable /position_cmd only after a goal")
    args = parser.parse_args(rospy.myargv()[1:])
    cfg["velocity"] = 1.0
    GazeboYopo(args.execute)
