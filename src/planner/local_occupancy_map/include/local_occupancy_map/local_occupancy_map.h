...
class LocalOccupancyMap {
 ...
 private:
  ...
  Eigen::Vector3d closestPointInMap(const Eigen::Vector3d& pt, const Eigen::Vector3d& camera_pt) const;
  void resetInflationBuffer(const Eigen::Vector3i& min_id, const Eigen::Vector3i& max_id);
  void inflatePoint(const Eigen::Vector3i& pt, int step, std::vector<Eigen::Vector3i>& pts) const;
  double logit(double p) const;
  void setOccupancyLogOdds(const Eigen::Vector3i& id, double update);
