...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="*",
        help="Goal position: x y [z]. If omitted, enter interactive input mode.",
    )
    parser.add_argument(
        "--default-z",
        type=float,
...
def publish_goal(pub, rate, args, x, y, z):
    ...
    rospy.loginfo(
        "Published goal to %s: frame=%s xyz=(%.2f, %.2f, %.2f) yaw=%.1f deg",
        args.topic,
        args.frame_id,
        x,
        y,
...
def interactive_loop(pub, rate, args):
    print("Input goal as: x y [z]. Press Ctrl-D or type q/quit/exit to stop.")
