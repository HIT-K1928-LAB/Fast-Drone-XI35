...
bool YopoEngine::build() {
    ...
    std::cout << "Failed to load YOPO engine, building from ONNX: " << config_.onnx_file
              << std::endl;
...
bool YopoEngine::infer(
    const std::array<float, 1 * 1 * 96 * 160>& depth, const std::array<float, 1 * 9 * 3 * 5>& obs,
    std::array<float, 1 * 9 * 3 * 5>* endstate, std::array<float, 1 * 3 * 5>* score) {
    if (!context_ || !endstate || !score) return false;

    if (!checkCuda(
            cudaMemcpy(
                depth_device_, depth.data(), depth.size() * sizeof(float), cudaMemcpyHostToDevice),
            "cudaMemcpy depth H2D")) {
        return false;
    }
    if (!checkCuda(
            cudaMemcpy(obs_device_, obs.data(), obs.size() * sizeof(float), cudaMemcpyHostToDevice),
            "cudaMemcpy obs H2D")) {
        return false;
    }
    ...
    if (!checkCuda(
            cudaMemcpy(
                endstate->data(), endstate_device_, endstate->size() * sizeof(float),
    ...
    if (!checkCuda(
            cudaMemcpy(
                score->data(), score_device_, score->size() * sizeof(float),
                cudaMemcpyDeviceToHost),
...
bool YopoEngine::buildFromOnnx() {
    ...
    if (!parser->parseFromFile(
            config_.onnx_file.c_str(), static_cast<int>(nvinfer1::ILogger::Severity::kWARNING))) {
...
bool YopoEngine::prepareBindings() {
    ...
    auto check_binding = [&](int index, size_t expected_volume, const char* label) -> bool {
        ...
        if (got != expected_volume) {
            std::cerr << label << " volume mismatch. got=" << got << " expected=" << expected_volume
                      << std::endl;
            return false;
        }
        return true;
    ...
    if (!checkCuda(
            cudaMalloc(&depth_device_, 1 * 1 * 96 * 160 * sizeof(float)), "cudaMalloc depth")) {
        return false;
    }
    ...
    if (!checkCuda(
            cudaMalloc(&endstate_device_, 1 * 9 * 3 * 5 * sizeof(float)), "cudaMalloc endstate")) {
        return false;
    }
