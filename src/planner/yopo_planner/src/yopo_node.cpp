...
class YopoPlannerNode {
 public:
  YopoPlannerNode() : nh_(), pnh_("~") {
    ...
    lattice_traj_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/yopo_net/lattice_trajs_visual", 1);

    odom_sub_ = nh_.subscribe(odom_topic, 1, &YopoPlannerNode::odomCallback, this,
                              ros::TransportHints().tcpNoDelay());
    depth_sub_ = nh_.subscribe(depth_topic, 1, &YopoPlannerNode::depthCallback, this,
                               ros::TransportHints().tcpNoDelay());
    goal_sub_ = nh_.subscribe("/move_base_simple/goal", 1, &YopoPlannerNode::goalCallback, this);
    ctrl_timer_ =
        nh_.createTimer(ros::Duration(planner_->params().ctrl_dt), &YopoPlannerNode::controlTimer, this);
 ...
 private:
  ...
  bool prepareDepth(const sensor_msgs::Image& msg,
                    std::array<float, 1 * 1 * 96 * 160>* depth_input) const {
    ...
    if (msg.encoding == "32FC1") {
      ...
      for (uint32_t r = 0; r < msg.height; ++r) {
        ...
        float* dst = depth_meters.ptr<float>(r);
        for (uint32_t c = 0; c < msg.width; ++c) dst[c] = static_cast<float>(src[c]) / 1000.0f;
      }
    } else {
    ...
    if (depth_meters.rows != params.image_height || depth_meters.cols != params.image_width) {
      cv::resize(depth_meters, resized, cv::Size(params.image_width, params.image_height), 0.0, 0.0,
                 cv::INTER_NEAREST);
    } else {
    ...
    for (int r = 0; r < params.image_height; ++r) {
      ...
      for (int c = 0; c < params.image_width; ++c) {
        ...
        v = std::min(v, static_cast<float>(params.max_depth)) / static_cast<float>(params.max_depth);
        dst[c] = v;
        ...
        for (int c = 0; c < current.cols; ++c) {
          ...
          for (int dr = -1; dr <= 1; ++dr) {
            for (int dc = -1; dc <= 1; ++dc) {
              ...
              const int rr = r + dr;
              const int cc = c + dc;
              if (rr < 0 || rr >= current.rows || cc < 0 || cc >= current.cols) continue;
  ...
  void printTiming(const Clock::time_point& t0, const Clock::time_point& t1,
                   const Clock::time_point& t2, const Clock::time_point& t3,
                   const Clock::time_point& t4, const Clock::time_point& t5) {
    ++count_;
    ...
    if (verbose_ || total > 1000.0 / depth_fps_) {
      ROS_INFO("YOPO avg ms: depth %.2f prepare %.2f infer %.2f post %.2f vis %.2f",
