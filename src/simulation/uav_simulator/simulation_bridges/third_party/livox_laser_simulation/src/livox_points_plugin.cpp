...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    const std::string file_name = ::simulation_bridges::resolvePackageUri(
        csv_uri, [](const std::string &package_name) { return ros::package::getPath(package_name); });
    ROS_INFO_STREAM("load csv file name:" << file_name);
    if (file_name.empty()) {
        ROS_ERROR_STREAM("cannot resolve csv file URI: " << csv_uri);
        return;
    }
