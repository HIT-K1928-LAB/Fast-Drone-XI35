...
class YoloTrtNode {
  public:
    YoloTrtNode(ros::NodeHandle& nh, ros::NodeHandle& pnh) : nh_(nh), pnh_(pnh), it_(nh_) {
        ...
        pnh_.param<std::string>(
            "camera_info_topic", camera_info_topic_, "/camera/infra1/camera_info");
        ...
        ROS_INFO_STREAM(
            ...
            << ", extrinsic_topic=" << extrinsic_topic_ << ", target_topic=" << target_topic_);
    }
  ...
  private:
    void loadEngine() {
        ...
        ROS_INFO_STREAM(
            "TensorRT engine loaded: input=1x3x" << input_h_ << "x" << input_w_ << ", output=1x"
    ...
    LetterboxInfo preprocess(const cv::Mat& image) {
        ...
        resized.copyTo(letterboxed(
            cv::Rect(
                static_cast<int>(info.pad_x), static_cast<int>(info.pad_y), resized_w, resized_h)));
    ...
    void publishTargetPoint(
        ...
        // point.header.frame_id = frame_id_.empty() ? (odom.header.frame_id.empty() ?
        // global_frame_id_ : odom.header.frame_id) : frame_id_; point.point.x = target_w.x();
        ...
        const tf2::Vector3 target_b(
            target_c.z() + camera_tx_, -target_c.x() + camera_ty_, -target_c.y() + camera_tz_);

        point.header          = header;
        point.header.frame_id = frame_id_.empty() ? "base_link" : frame_id_;

        point.point.x = target_b.x();
        point.point.y = target_b.y();
        point.point.z = target_b.z();
