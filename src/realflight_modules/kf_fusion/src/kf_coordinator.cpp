...
Eigen::Matrix<T, 3, 3> skew(const Eigen::Matrix<T, 3, 1>& vec) {
    return (Eigen::Matrix<T, 3, 3>() << T(0), -vec(2), vec(1), vec(2), T(0), -vec(0), -vec(1),
            vec(0), T(0))
        .finished();
}
...
void KfCoordinator::insertStateWithCov(
    UniID state_id, const double* data, StateType state_type, const Eigen::MatrixXd& init_cov) {
    auto idx_size_opt = state_manager_.insertState(state_id, state_type, data, false);
    if (idx_size_opt.has_value()) {
        ...
        if (init_cov.rows() != local_size || init_cov.cols() != local_size) {
            std::cerr << "init_cov's Dim is wrong! target state is" << static_cast<int>(state_type)
                      << std::endl;
            return;
        }
        state_cov_.block(start_index, start_index, local_size, local_size) = init_cov;
        ROS_DEBUG(
            "InserState: state type %d, start index %d, local size %d.", state_type, start_index,
            local_size);
    } else {
...
void KfCoordinator::insertConstState(UniID state_id, const double* data, StateType state_type) {
    auto idx_size_opt = state_manager_.insertState(state_id, state_type, data, true);
    if (!idx_size_opt.has_value()) {
        std::cerr << "insert state wrong!" << std::endl;
        return;
    }
    auto [start_index, local_size] = idx_size_opt.value();
    ROS_DEBUG(
        "InserConstState: state type %d, start index %d, local size %d.", state_type, start_index,
        local_size);
    return;
}
...
void KfCoordinator::init(const StateInitializer::StateInit& state_init) {
    ...
    if (kf_config.estimate_ext_rot) {
        ...
        insertStateWithCov(
            getExtOdomOriUid(), state_init.q_i_b.coeffs().data(), StateType::kExtOdomOri,
            ext_rot_cov_mat);
    } else {
        insertConstState(
            getExtOdomOriUid(), state_init.q_i_b.coeffs().data(), StateType::kExtOdomOri);
    ...
    if (kf_config.estimate_ext_tvec) {
        ...
        insertStateWithCov(
            getExtOdomTvecUid(), state_init.p_i_b.data(), StateType::kExtOdomTvec,
            ext_tvec_cov_mat);
    } else {
        insertConstState(getExtOdomTvecUid(), state_init.p_i_b.data(), StateType::kExtOdomTvec);
    }
    is_init_ = true;
...
void KfCoordinator::processImuMeas(const ImuMeasPtr& imu_meas) {
    ...
    imuStatePropagate(imu_meas->timestamp_s, imu_meas->gyr_rad_s, imu_meas->acc_m_s2);
}
void KfCoordinator::imuStatePropagate(
    double cur_imu_time, const Eigen::Vector3d& cur_gyro, const Eigen::Vector3d& cur_accel) {
    ...
    F.block<3, 3>(3, 0) =
        -0.25 * state_R0 * R_a_0_x * dt * dt +
        -0.25 * state_R1 * R_a_1_x * (Eigen::Matrix3d::Identity() - R_w_x * dt) * dt * dt;
    ...
    F.block<3, 3>(6, 0) =
        -0.5 * state_R0 * R_a_0_x * dt +
        -0.5 * state_R1 * R_a_1_x * (Eigen::Matrix3d::Identity() - R_w_x * dt) * dt;
    ...
    state_cov_.block<15, 15>(0, 0) =
        F * state_cov_.block<15, 15>(0, 0) * F.transpose() + V * imu_noise_cov_ * V.transpose();
...
void KfCoordinator::addResidualJacobian(
    const Eigen::VectorXd& residual, const std::vector<Eigen::MatrixXd>& jacobians,
    const std::vector<IdxSizePair>& id_size_pairs, const DUMatType& du_jacobian) {
    int res_dim                  = static_cast<int>(residual.size());
    Eigen::MatrixXd jacobian_aug = Eigen::MatrixXd::Zero(res_dim, total_local_size);
...
DelayUpdateInfo KfCoordinator::getDelayUpdateInfo(double delay_update_time) {
    ...
    for (auto it = imu_que_.rbegin(); it != imu_que_.rend(); it++) {
        if ((*it)->timestamp_s > delay_update_time) {
            ...
            auto it_1      = it - 1;
            auto dummy_imu = ImuHelper::imuLinearInterPolation(*it, *it_1, delay_update_time);
    ...
    double t_i_j   = preintg_que.back()->timestamp_s - preintg_que.front()->timestamp_s;
    ...
    const Eigen::Vector3d vec_temp_0 = deltaR.transpose() * (deltaV * t_i_j - deltaP);
...
void KfCoordinator::solveAndUpdate() {
    ...
    std::vector<IdxSizePair> const_id_size = state_manager_.getConstStateIdxSize();
    ...
    const Eigen::MatrixXd kf_temp =
        Eigen::MatrixXd::Identity(total_local_size, total_local_size) - kf_K * kf_H;
    state_cov_ = kf_temp * state_cov_ * kf_temp.transpose() + kf_K * obv_R * kf_K.transpose();
...
void KfCoordinator::updateWithOdomMeas(const OdomMeasPtr& odom_meas) {
    ...
    const Eigen::Vector3d gravity(0, 0, -kGravityMag);
    ROS_DEBUG("updateWithOdomMeas: t:%f, deltaT:%f.", odom_meas->timestamp_s, du_info.deltaT);
    // rot residual
    ...
    int rot_wi_local_size      = state_manager_.state_pool.at(getImuOriUid()).localSize();
    int rot_ib_idx             = state_manager_.state_pool.at(getExtOdomOriUid()).startIndex();
    int rot_ib_local_size      = state_manager_.state_pool.at(getExtOdomOriUid()).localSize();
    ...
    addResidualJacobian(residual_rot, jacobians, id_size_pairs, du_info.d_si_d_sj);
    jacobians.clear();
    id_size_pairs.clear();
    ...
    int posi_ib_idx            = state_manager_.state_pool.at(getExtOdomTvecUid()).startIndex();
    int posi_ib_local_size     = state_manager_.state_pool.at(getExtOdomTvecUid()).localSize();
    ...
    addResidualJacobian(residual_posi, jacobians, id_size_pairs, du_info.d_si_d_sj);
    jacobians.clear();
    id_size_pairs.clear();
    ...
    int vel_w_local_size     = state_manager_.state_pool.at(getImuVelUid()).localSize();
    ...
    addResidualJacobian(residual_vel, jacobians, id_size_pairs, du_info.d_si_d_sj);
    jacobians.clear();
    id_size_pairs.clear();

    if (kf_config.use_motion_capture) {
        obv_R                   = Eigen::Matrix<double, 9, 9>::Identity();
        obv_R.block<3, 3>(0, 0) = kf_config.obv_mc_rot_cov.array().square().matrix().asDiagonal();
        obv_R.block<3, 3>(3, 3) = kf_config.obv_mc_posi_cov.array().square().matrix().asDiagonal();
        obv_R.block<3, 3>(6, 6) = kf_config.obv_mc_vel_cov.array().square().matrix().asDiagonal();
    } else if (kf_config.enable_merge_vins_bias) {
        // bias accel residual
        Eigen::Vector3d state_bias_accel    = state_manager_.getBiasAccelState();
        Eigen::Vector3d residual_bias_accel = odom_meas->bias_accel - state_bias_accel;
        // bias accel jacobian
        int bias_accel_idx        = state_manager_.state_pool.at(getImuBiasAccelUid()).startIndex();
        int bias_accel_local_size = state_manager_.state_pool.at(getImuBiasAccelUid()).localSize();
        ...
        addResidualJacobian(residual_bias_accel, jacobians, id_size_pairs, du_info.d_si_d_sj);

        // set obv_R(fixed)
        obv_R                   = Eigen::Matrix<double, 12, 12>::Identity();
        obv_R.block<3, 3>(0, 0) = kf_config.obv_odom_rot_cov.array().square().matrix().asDiagonal();
        obv_R.block<3, 3>(3, 3) =
            kf_config.obv_odom_posi_cov.array().square().matrix().asDiagonal();
        obv_R.block<3, 3>(6, 6) = kf_config.obv_odom_vel_cov.array().square().matrix().asDiagonal();
        obv_R.block<3, 3>(9, 9) =
...
void KfCoordinator::updateWithZUPT(const ImuMeasPtr& imu_meas) {
    ...
    // gyro jacobian
    int bias_gyro_idx         = state_manager_.state_pool.at(getImuBiasGyroUid()).startIndex();
    int bias_gyro_local_size  = state_manager_.state_pool.at(getImuBiasGyroUid()).localSize();
    ...
    addResidualJacobian(residual_gyro, jacobians, id_size_pairs, DUMatType::Identity());
    jacobians.clear();
    id_size_pairs.clear();
    ...
    Eigen::Vector3d residual_accel =
        -(R_w_i * (imu_meas->acc_m_s2 - state_bias_accel) + Eigen::Vector3d(0, 0, -kGravityMag));
    // accel jacobian
    ...
    id_size_pairs.emplace_back(bias_accel_idx, bias_accel_local_size);
    addResidualJacobian(residual_accel, jacobians, id_size_pairs, DUMatType::Identity());
    jacobians.clear();
    id_size_pairs.clear();
    ...
    addResidualJacobian(residual_v, jacobians, id_size_pairs, DUMatType::Identity());

    // set obv_R
    obv_R                   = Eigen::Matrix<double, 9, 9>::Identity();
    obv_R.block<3, 3>(0, 0) = kf_config.obv_zupt_gyro_cov.array().square().matrix().asDiagonal();
    obv_R.block<3, 3>(3, 3) = kf_config.obv_zupt_accel_cov.array().square().matrix().asDiagonal();
    obv_R.block<3, 3>(6, 6) = kf_config.obv_zupt_vel_cov.array().square().matrix().asDiagonal();

    solveAndUpdate();
}

void KfCoordinator::updateWithTofMeas(const TofMeasPtr& tof_meas) {
    double d = tof_meas->distance;
    if (d > 0.0201) {
        DelayUpdateInfo du_info = getDelayUpdateInfo(tof_meas->timestamp_s);
        const Eigen::Vector3d gravity(0, 0, -kGravityMag);
        std::vector<Eigen::MatrixXd> jacobians;
        std::vector<IdxSizePair> id_size_pairs;
        double z0                   = 0.06;  // TOF IMU
        double z2                   = 0.001;
        Eigen::Vector3d state_p_w_i = state_manager_.getImuTvecState();
        Sophus::SO3d state_so3_w_i  = state_manager_.getImuOriState();
        state_so3_w_i *= Sophus::SO3d(du_info.deltaR.transpose());
        ...
        R_i_f << 1, 0, 0, 0, -1, 0, 0, 0, -1;
        Eigen::Vector3d t_i_f;
        t_i_f << -0.05, -0.05, -0.06;
        // t_i_f1 = R_w_i * t_i_f;
        // z4 = -t_i_f1.z();
        ...
        addResidualJacobian(residual_h, jacobians, id_size_pairs, du_info.d_si_d_sj);
        jacobians.clear();
        id_size_pairs.clear();
