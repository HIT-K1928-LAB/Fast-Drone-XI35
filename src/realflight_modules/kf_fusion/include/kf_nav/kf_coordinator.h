...
class KfCoordinator {
  public:
    struct Config {
        ...
        Eigen::Vector3d obv_tof_h_cov;

        void loadConfig() {
            accel_noise_std = ParamReader::getInstance().getDouble("accel_noise_std", 0.1);
            gyro_noise_std  = ParamReader::getInstance().getDouble("gyro_noise_std", 0.02);
            accel_walk_std  = ParamReader::getInstance().getDouble("accel_walk_std", 1e-3);
            gyro_walk_std   = ParamReader::getInstance().getDouble("gyro_walk_std", 1e-4);

            imu_ori_init_cov   = ParamReader::getInstance().getDouble("imu_ori_init_cov", 4e-5);
            imu_yaw_init_cov   = ParamReader::getInstance().getDouble("imu_yaw_init_cov", 1e-5);
            imu_posi_init_cov  = ParamReader::getInstance().getDouble("imu_posi_init_cov", 1e-5);
            imu_vel_init_cov   = ParamReader::getInstance().getDouble("imu_vel_init_cov", 0.09);
            gyro_bias_init_cov = ParamReader::getInstance().getDouble("gyro_bias_init_cov", 1e-7);
            acc_bias_init_cov  = ParamReader::getInstance().getDouble("acc_bias_init_cov", 1e-2);
            ext_ori_init_cov   = ParamReader::getInstance().getDouble("ext_ori_init_cov", 1e-6);
            ext_posi_init_cov  = ParamReader::getInstance().getDouble("ext_posi_init_cov", 1e-4);

            use_motion_capture = ParamReader::getInstance().getBool("use_motion_capture", false);
            estimate_ext_rot   = ParamReader::getInstance().getBool("estimate_ext_rot", false);
            ...
            obv_tof_h_cov = ParamReader::getInstance().getFixedVector<double, 3>(
                "obv_tof_h_cov", Eigen::Vector3d::Identity());
        }
    };
    Config kf_config;
  ...
  public:
    KfCoordinator() {
        ...
        imu_noise_cov_.block<3, 3>(0, 0) =
            (kf_config.accel_noise_std * kf_config.accel_noise_std) * Eigen::Matrix3d::Identity();
        imu_noise_cov_.block<3, 3>(3, 3) =
            (kf_config.gyro_noise_std * kf_config.gyro_noise_std) * Eigen::Matrix3d::Identity();
        imu_noise_cov_.block<3, 3>(6, 6) =
            (kf_config.accel_noise_std * kf_config.accel_noise_std) * Eigen::Matrix3d::Identity();
        imu_noise_cov_.block<3, 3>(9, 9) =
            (kf_config.gyro_noise_std * kf_config.gyro_noise_std) * Eigen::Matrix3d::Identity();
        imu_noise_cov_.block<3, 3>(12, 12) =
            (kf_config.accel_walk_std * kf_config.accel_walk_std) * Eigen::Matrix3d::Identity();
        imu_noise_cov_.block<3, 3>(15, 15) =
            (kf_config.gyro_walk_std * kf_config.gyro_walk_std) * Eigen::Matrix3d::Identity();
    }

    void insertStateWithCov(
        UniID state_id, const double* data, StateType state_type, const Eigen::MatrixXd& init_cov);
    void insertConstState(UniID state_id, const double* data, StateType state_type);
