...
class ImuHelper {
  public:
    struct PreintegrationRes {
        ...
        PreintegrationRes(
            const Eigen::Matrix3d &deltaR_, const Eigen::Vector3d &deltaP_,
    ...
    static ImuMeasPtr imuLinearInterPolation(
        const ImuMeasPtr &imu0, const ImuMeasPtr &imu1, double t_intp) {
        double coffe_0   = (imu1->timestamp_s - t_intp) / (imu1->timestamp_s - imu0->timestamp_s);
        double coffe_1   = (t_intp - imu0->timestamp_s) / (imu1->timestamp_s - imu0->timestamp_s);
...
class StaticCheck {
  public:
    void inputImu(const ImuMeasPtr &imu) {
        if (!imu_que_.empty() && imu->timestamp_s < imu_que_.back()->timestamp_s) return;
    ...
    bool checkStatic() {
        ...
        double accel_var = 0.;
        for (auto &imu_ptr : imu_que_) {
            gyro_var += (imu_ptr->gyr_rad_s - gyro_avg).dot(imu_ptr->gyr_rad_s - gyro_avg);
            accel_var += (imu_ptr->acc_m_s2 - accel_avg).dot(imu_ptr->acc_m_s2 - accel_avg);
        }
